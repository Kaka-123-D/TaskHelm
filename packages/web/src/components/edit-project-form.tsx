...
interface FormState {
  ...
  readonly maxActiveDevServers: string
}
...
export function EditProjectForm({ project }: EditProjectFormProps) {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    maxActiveDevServers: String(project.max_active_dev_servers),
  })
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (form.maxActiveDevServers) body.max_active_dev_servers = parseInt(form.maxActiveDevServers, 10)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="space-y-3">
          <Field label="Name *" value={form.name} onChange={updateField('name')} placeholder="Project name" />
          ...
          <Field label="Max Dev Servers" value={form.maxActiveDevServers} onChange={updateField('maxActiveDevServers')} placeholder="3" />
        </div>
